...
BOOST_AUTO_TEST_CASE(rpc_z_mergetoaddress_parameters)
{
    ...
    try {
        std::shared_ptr<AsyncRPCOperation> operation(new AsyncRPCOperation_mergetoaddress({}, {}, testnetzaddr, -1 ));
    ...
    try {
        ...
        std::shared_ptr<AsyncRPCOperation> operation(new AsyncRPCOperation_mergetoaddress(inputs, {}, badaddr, 1));
        BOOST_FAIL("Should have caused an error");
    } catch (const UniValue& objError) {
    ...
    try {
        std::vector<MergeToAddressInputUTXO> inputs = { MergeToAddressInputUTXO{ COutPoint{uint256(), 0}, 0} };
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_mergetoaddress(inputs, {}, mainnetzaddr, 1) );
        BOOST_FAIL("Should have caused an error");
    } catch (const UniValue& objError) {
        BOOST_CHECK( find_error(objError, "payment address is for wrong network type"));
    }
...
BOOST_AUTO_TEST_CASE(rpc_z_mergetoaddress_internals)
{
    ...
    // Supply 2 inputs when mempool limit is 1
    {
        ...
        std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_mergetoaddress(inputs, {}, zaddr1) );
        operation->main();
        BOOST_CHECK(operation->isFailed());
        std::string msg = operation->getErrorMessage();
